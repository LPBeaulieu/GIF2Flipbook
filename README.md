...
- <b>For best results, generate flipbooks from 5 to 6 second video clips, with an fps of 25. This results in flipbooks of a thickness of around 1.5-1.8 cm (0.6-0.7 inch) when printed on 24 lb multipurpose paper, which is a manageable thickness for optimal page flicking. A minimum weight of 24 lb should be used when printing on inkjet printers, so as to minimize any ink bleeding onto the other side of the page, as the flipbooks are to be printed in duplex mode when more than 4 flipbooks are generated at a time. Also, when printing flipbooks on laser printers, the toner density setting may need to be decreased to avoid printing too dark. In my experience, MP4 videos work better than GIFs, as some GIFs have very low frame rates and total number of frames. Also, using MP4s with a resolution of at least 720p will lead to higher quality printed images and MP4s with an aspect ratio of 16:9 (such as in 720p or 1080p resolutions) will make it easier to see the whole frame when flipping the flipbooks, as the frames aren't too tall.</b>
...
- <b> For best results, it is recommended to use a stack paper cutter when assembling the flipbooks</b>, to ensure proper page alignment. First cut the stack of flipbook pages in half alongside the central horizontal and vertical axes of the 8 1/2 by 11 inch pages, thus giving four flipbooks. Then bind the flipbooks using using PVA glue (see the figures 9 to 26 of my other github <b>PrintABook</b> for more details on this: https://github.com/LPBeaulieu/Book-Generator-PrintABook). Finally, trim the flipping edges of the bound flipbooks with the stack paper cutter to make sure that the flipbooks flip as smoothly as possible. <b>You might need to shift the frames away from the flipping edges of the flipbooks, to allow for sufficient space to trim the flipping edges with the stack paper cutter. If so, you can shift the frames upwards by passing in the vertical shift in inches (in decimal form and without units) after the "vertical_shift:" argument</b>. The default bottom margin is 0.25 inch, so check your stack paper cutter's specifications to find out if that is sufficient for trimming the bottom edges
of the flipbooks. If  not, then any value passed in after the "vertical_shift:" argument would be added to the bottom margin. For example, with the default margin of 0.25 inch and a vertical shift of 0.25 inch, there would be 0.5 inch of white space between the flipping edges of the flipbook pages and the bottom of the frames.
...
- Here are some instructions on how to print 3D anaglyph flipbooks (you will require red-blue 3D glasses):
  ...
  - A GIF will be created for each animation, allowing you to view it, in order to make sure that the settings are right before printing your flipbook. 
...
- Once you are done printing and cutting your pages, you can perforate the pages and bind them with 1 inch plastic binder rings. Despite my shoddy scissor work, the flipbooks still turned out great! <b>For smoother page flicking though, I would suggest actually binding your flipbooks using PVA glue</b> (see the figures 9 to 26 of my other github <b>PrintABook</b> for more details on this: https://github.com/LPBeaulieu/Book-Generator-PrintABook)
